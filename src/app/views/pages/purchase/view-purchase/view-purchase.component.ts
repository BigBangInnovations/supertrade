...
export class ViewPurchaseComponent implements OnInit, OnDestroy {
	...
	getTitle(): string {
		...
		if (this.pageAction == "purchaseReturn") {
			...
			this.pageAction == "retailerPurchaseReturnApprovalByDistributor"
		) {
	...
	/**
	 * acceptPurchaseReturnByDistributor
	 */
	acceptPurchaseReturnByDistributor() {
		this.enableAttributes();
		const controls = this.purchaseForm.controls;
		...
		this.data.notificationData[0].Status = 2;
		const returnPurchase = this.preparePurchase();
		this.acceptRejectPurchaseReturnByDistributor(returnPurchase);
	}

	/**
	 * rejectPurchaseReturnByDistributor
	 */
	rejectPurchaseReturnByDistributor() {
		this.enableAttributes();
		const controls = this.purchaseForm.controls;
	...
	preparePurchase(): Purchase {
		...
		_purchase.distributor_id = this.purchaseForm.controls[
			"distributor_id"
		].value;
		...
		if (
			...
			_purchase.retailer_name = this.purchaseForm.controls[
				"retailer_name"
			].value;
			_purchase.Distributor_Name = this.purchaseForm.controls[
				"distributor_name"
			].value;
		}
	...
	/**
	 * acceptRejectPurchaseReturnByDistributor
	 *
	 * @param _purchase: User
	 */
	acceptRejectPurchaseReturnByDistributor(_purchase: Purchase) {
		this.loading = true;
		let httpParams = new HttpParams();
		Object.keys(_purchase).forEach(function(key) {
			httpParams = httpParams.append(key, _purchase[key]);
		...
		this.purchaseService
			.acceptRejectPurchaseReturnByDistributor(httpParams)
			.pipe(
				tap(response => {
					if (response.status == APP_CONSTANTS.response.SUCCESS) {
						const message = `Purchase return approval updated successfully.`;
